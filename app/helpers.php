<?php
global $webInfo;
$catId = GetCatId();


/**
 * 获取类目ID
 * @return int
 */
function GetCatId()
{
    $pathArr = \App\Service\Utils::getPath();
    $rPath = array_pop($pathArr);
    if (ctype_digit($rPath)) {
        return $catId = $rPath;
    } else {
        $ret = \App\Models\Category::getCatIdByPath($rPath);
        return $catId = $ret['id'];
    }
}

/**
 * 获取文章ID
 * @return int
 */
function GetArticleId()
{
    $path = \Illuminate\Support\Facades\Input::path();
    $arrPath = explode('/', $path);
    if (preg_match('/html$/', $path)) {
        return $id = (int)array_pop($arrPath);
    }
    return 0;
}

/**
 * 获取当前分页数
 * @return mixed
 */
function GetPageNum()
{
    return \Illuminate\Support\Facades\Input::get('page');
}

/**
 * 获取搜索关键字，即搜索框中提交的文字
 * @return mixed
 */
function GetSearchKey()
{
    return \Illuminate\Support\Facades\Input::get('keyword');
}

/**
 * 网站基本信息
 * @param  string $param 参数有logo,url,,seo_title,keyword,describe,copyright,hotline,record,traffic_statistics,online_qq,contact,email,phone,address
 * @return mixed
 */
function WebInfo($param)
{
    return \App\Models\WebInfo::select($param)->first()[$param];
}


/**
 * 导航栏
 * @return array
 */
function NavBar()
{
    $list = \App\Models\Category::lists();
    $arr = \App\Service\Utils::categoryTree($list);
    return $arr;
}


/**
 * 侧边栏
 * @return array
 */
function SideBar()
{

    $getPath = \App\Service\Utils::getPath();

    $path = array_shift($getPath);

    $category = \App\Models\Category::getCatIdByPath($path);

    $list = \App\Models\Category::subCategory($category->id)->toArray();

    $arr = \App\Service\Utils::categoryTree($list, '/' . $path);
    return $arr;
}

/**
 * 上部边栏
 * @return array
 */
function TopBar()
{

    $getPath = \App\Service\Utils::getPath();

    if (count($getPath) == 1) {
        return [];
    }

    $mainPath = array_shift($getPath);
    $path = array_shift($getPath);

    $category = \App\Models\Category::getCatIdByPath($path);

    $list = \App\Models\Category::subCategory($category->id)->toArray();

    $arr = \App\Service\Utils::categoryTree($list, '/' . $mainPath . '/' . $path);
    return $arr;
}


/**
 * 导航高亮显示
 * @param $currLink
 * @param $className
 * @return string
 */
function Active($currLink, $className)
{
    $path = $_SERVER['REQUEST_URI'];

    if ($path == '/' && $currLink == '/') {
        return "class={$className}";
    }

    if (preg_match_all('/(.*)\/(.*)(\.html)/', $path, $matches)) {
        $path = $matches[1][0];
    }

    if (strpos($path, $currLink) !== false && $currLink != '/') {
        return "class={$className}";
    }

    return '';

}

/**
 * 获取指定类目的所有文章，带分页
 * @param int $catId 栏目ID，0值代表不分类目，读取所有文章
 * @param string $search 搜索
 * @param int $perPage 每页显示条数
 * @param int $sort 排序 0：正序，1：倒序
 * @param string $field 排序使用的字段名，用于排序，只能是id，sort，update，top，recommended，visits等字段。id：按文章添加顺序，sort：按指定顺序排序，update，按更新时间排序
 * @return array
 */

function ArticleAll($catId = 0, $search = '', $perPage = 10, $sort = 1, $field = 'id')
{
    $pageNum = GetPageNum();
    $article = \App\Models\Article::search($catId, $search, $pageNum, $perPage, $sort, $field);
    global $page;

    $page = $article['page'];
    return $article['list'];
}

/**
 * 分页
 * @return mixed
 */
function Page()
{
    return $GLOBALS['page'];
}


/**
 * 获取栏目信息
 * @param $catId
 * @return mixed
 */
function CatInfo($catId = '')
{
    if ($catId == '') {
        $catId = GetCatId();
    }
    return \App\Models\Category::getCatById($catId);
}


/**
 * 获取文章内容
 * @param $catId
 * @return mixed
 */
function ArticleInfo($catId = 0)
{
    if ($catId == 0) {
        $catId = GetArticleId();
    }
    $article = \App\Models\Article::getInfoByCatId($catId);
    if (!empty($article->id)) {
        $article->visits = $article->visits + 1;
        $article->update();
    }

    return $article;
}


/**
 * 首页轮播图
 */
function Banner()
{
    return \App\Models\Banner::select(['id', 'title', 'picture', 'link'])->where('state', 1)->orderBy('sort', 'DESC')->get();
}


/**
 * 获取指定类目第一篇文章
 * @param $catId int 栏目ID，0值代表不分类目，读取第一条录入的文章
 * @return array
 */
function FirstArticle($catId)
{
    return ArticleList($catId, 1, 0);
}

/**
 * 获取指定类目最后一篇文章
 * @param $catId int 栏目ID，0值代表不分类目，读取最后一条录入的文章
 * @return array
 */
function LastArticle($catId)
{
    return ArticleList($catId, 1, 1);
}

/**
 * 获取指定类目置顶文章
 * @param $catId int 栏目ID，0值代表不分类目，读取所有置顶文章
 * @param int $num 读取条数
 * @param int $sort 排序 0：正序，1：倒序
 * @param string $field 排序使用的字段名，用于排序，只能是id，sort，update，top，recommended，visits等字段。id：按文章添加顺序，sort：按指定顺序排序，update，按更新时间排序
 * @return array
 */
function ArticleTop($catId, $num = 10, $sort = 1, $field = 'id')
{
    return \App\Models\Article::lists($catId, $num, $sort, $field, 'placed_top');
}

/**
 * 获取指定类目滚动文章
 * @param $catId int 栏目ID，0值代表不分类目，读取所有滚动文章
 * @param int $num 读取条数
 * @param int $sort 排序 0：正序，1：倒序
 * @param string $field 排序使用的字段名，用于排序，只能是id，sort，update，top，recommended，visits等字段。id：按文章添加顺序，sort：按指定顺序排序，update，按更新时间排序
 * @return array
 */
function ArticleRoll($catId, $num = 10, $sort = 1, $field = 'id')
{
    return \App\Models\Article::lists($catId, $num, $sort, $field, 'rolling');
}

/**
 * 获取指定类目推荐文章
 * @param int $catId 栏目ID，0值代表不分类目，读取所有推荐文章
 * @param int $num 读取条数
 * @param int $sort 排序 0：正序，1：倒序
 * @param string $field 排序使用的字段名，用于排序，只能是id，sort，update，top，recommended，visits等字段。id：按文章添加顺序，sort：按指定顺序排序，update，按更新时间排序
 * @return array
 */
function ArticleRec($catId, $num = 10, $sort = 1, $field = 'id')
{
    return \App\Models\Article::lists($catId, $num, $sort, $field, 'recommended');
}

/**
 * 获取指定类目指定条数的文章
 * @param $catId int 栏目ID，0值代表不分类目，读取限定条数的文章
 * @param int $num 读取条数
 * @param int $sort 排序 0：正序，1：倒序
 * @param string $field 排序使用的字段名，用于排序，只能是id，sort，update，top，recommended，visits等字段。id：按文章添加顺序，sort：按指定顺序排序，update，按更新时间排序
 * @return array
 */
function ArticleList($catId, $num = 10, $sort = 1, $field = 'id')
{
    return \App\Models\Article::lists($catId, $num, $sort, $field);
}


/**
 * 上一条文章
 * @param string $catId
 * @return mixed
 */
function PrevTile($catId = '')
{
    $id = GetArticleId();
    if ($catId == '') {
        $catId = GetCatId();
    }
    $ret = \App\Models\Article::prevTile($catId, $id);
    $info = ['id' => $ret['id'] . '.html', 'title' => $ret['title']];
    return $info;
}

/**
 * 下一条文章
 * @param string $catId
 * @return mixed
 */
function NextTile($catId = '')
{
    $id = GetArticleId();
    if ($catId == '') {
        $catId = GetCatId();
    }
    $ret = \App\Models\Article::nextTile($catId, $id);
    return ['id' => $ret['id'] . '.html', 'title' => $ret['title']];
}

/**
 * 字符串截断
 * @param $str
 * @param $len
 * @param string $end
 * @return string
 */
function StrLimit($str, $len, $end = '...')
{
    return \Illuminate\Support\Str::limit($str, $len, $end);
}


/**
 * 时间转换
 * @param $time
 * @param string $format Y-m-d，例（2019-10-09），Y年m月d日 H时i分s秒，例（2019年10月09日 05时30分29秒）
 * @return string
 */
function TimeConvert($time, $format = 'Y-m-d')
{
    return date($format, strtotime($time));
}

/**
 * 获取自定义数据
 * @param  array $key
 * @return array
 * */
function GetCustomData($key)
{
    return \App\Models\Custom::select(['name', 'picture', 'link', 'content'])->where('name', $key)->first();
}

/**
 * 面包屑导航
 * @return array
 */
function Position()
{

    $qb = \App\Models\Category::getSubCatByPath(\App\Service\Utils::getPath());

    $arr = [];
    $path = '';
    if (!empty($qb)) {
        foreach ($qb as $item) {
            $path .= '/' . $item['dir_name'];
            $arr[] = ['name' => $item['name'], 'link' => $path];
        }
    }

    return $arr;
}

/**
 * 语言列表
 * @return mixed
 */
function Lang()
{
    return \App\Models\Language::select(['id', 'name', 'pic'])->where('state', 1)->get();
}






